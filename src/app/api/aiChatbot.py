...
async def call_ai(message, conv_id):
    ...
    if conv_id: 
        ...
        new_conversation = [{"role": msg["role"], "content": msg["message"]} for msg in conversation["message"]]
    ...
    messages = [ {"role": "user", "message": message}, {"role": chat_completion.choices[0].message.role,
                             "message": chat_completion.choices[0].message.content}]
